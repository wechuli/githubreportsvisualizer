...
interface ServiceChartProps {
  data: ServiceData[];
  title: string;
  serviceType: "actionsMinutes" | "actionsStorage" | "packages" | "copilot" | "codespaces";
  useSkuAnalysis?: boolean; // Override to use SKU-based analysis instead of repository-based
}

const COLORS = ["#22c55e", "#3b82f6", "#f59e0b", "#ef4444", "#8b5cf6", "#ec4899", "#06b6d4", "#84cc16", "#f97316", "#d946ef", "#64748b"];

export function ServiceChart({ data, title, serviceType, useSkuAnalysis = false }: ServiceChartProps) {
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 text-gray-500">
        <div className="text-center">
          <p className="text-lg font-medium">No data available</p>
          <p className="text-sm">No {title.toLowerCase()} usage found in your billing report</p>
        </div>
      </div>
    );
  }
  ...
  if (serviceType === "actionsMinutes" && !useSkuAnalysis) {
    // Check if data is from a single organization (indicating organization filter is active)
    const organizations = Array.from(new Set(data.map(item => item.organization).filter(Boolean)));
    const isSingleOrganization = organizations.length === 1;
    
    if (isSingleOrganization) {
      return <ActionsMinutesDetailedChart data={data} title={title} serviceType={serviceType} />;
    }
  }

  // Use repository-based analysis for Actions services (unless override), SKU-based for others
  const shouldUseRepositoryAnalysis = !useSkuAnalysis && (serviceType === "actionsMinutes" || serviceType === "actionsStorage");

  if (shouldUseRepositoryAnalysis) {
    return <RepositoryBasedChart data={data} title={title} serviceType={serviceType} />;
  } else {
    return <SKUBasedChart data={data} title={title} serviceType={serviceType} />;
  }
}

function ActionsMinutesDetailedChart({ data, title, serviceType }: ServiceChartProps) {
  ...
  const dailyData = data.reduce((acc, item) => {
    ...
    acc[date][`${repo}_quantity`] = (acc[date][`${repo}_quantity`] || 0) + item.quantity;
  ...
  const repoBreakdown = [...topRepos.map(repo => ({
    name: repo,
  ...
  const otherRepos = Object.keys(repoTotals).filter(repo => !topRepos.includes(repo));
  if (otherRepos.length > 0) {
    const othersCost = otherRepos.reduce((sum, repo) => sum + repoTotals[repo].cost, 0);
    const othersQuantity = otherRepos.reduce((sum, repo) => sum + repoTotals[repo].quantity, 0);
    repoBreakdown.push({
      name: 'Others',
      cost: othersCost,
  ...
  const uniqueRepos = new Set(data.map(item => item.repository).filter(Boolean)).size;
  ...
  const reposToShow = topRepos.length > 0 ? [...topRepos, ...(otherRepos.length > 0 ? ['Others'] : [])] : [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-gray-800/50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-blue-400">{formatQuantity(totalQuantity)}</p>
        </div>
        <div className="bg-gray-800/50 rounded-lg p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-gray-800/30 rounded-lg p-6">
          <h3 className="text-lg font-semibold mb-4">Daily Cost by Repository</h3>
          ...
          <div className="mt-4 space-y-2 max-h-32 overflow-y-auto">
            {skuBreakdown.map((entry, index) => (
              <div key={entry.name} className="flex items-center text-sm">
                ...
                />
                <span className="text-gray-300 flex-1 truncate" title={entry.name}>
                  {entry.name}
                </span>
                <span className="text-white font-medium">{formatCurrency(entry.cost)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800/30 rounded-lg p-6">
          <h3 className="text-lg font-semibold mb-4">Daily Minutes by Repository</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={stackedChartData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
              <XAxis dataKey="date" stroke="#9ca3af" fontSize={12} />
              <YAxis stroke="#9ca3af" fontSize={12} tickFormatter={formatQuantity} />
              <Tooltip
                contentStyle={{
                  backgroundColor: "#1f2937",
                  border: "1px solid #374151",
                ...
                formatter={(value: number) => [formatQuantity(value), "Minutes"]}
                labelStyle={{ color: "#d1d5db" }}
              />
              {reposToShow.map((repo, index) => (
                <Bar
                  ...
                  stackId="1"
                  fill={COLORS[index % COLORS.length]}
                  radius={index === reposToShow.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]}
                />
              ))}
            </BarChart>
          </ResponsiveContainer>
...
function RepositoryBasedChart({ data, title, serviceType }: ServiceChartProps) {
  // Get top 10 repositories by total cost
  const repoTotals = data.reduce((acc, item) => {
    const repo = item.repository || 'Unknown';
    if (!acc[repo]) {
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-700">
                <th className="text-left py-3 px-4 text-gray-300">Repository</th>
                <th className="text-right py-3 px-4 text-gray-300">Cost</th>
                <th className="text-right py-3 px-4 text-gray-300">Usage</th>
                <th className="text-right py-3 px-4 text-gray-300">% of Total</th>
              </tr>
            </thead>
            <tbody>
              {repoBreakdown.map((repo, index) => (
                <tr key={repo.name} className="border-b border-gray-800 hover:bg-gray-800/50">
      ...
      {/* Daily Usage Volume - Stacked by Repository */}
      <div className="bg-gray-800/30 rounded-lg p-6">
        <h3 className="text-lg font-semibold mb-4">Daily Usage Volume by Repository</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={stackedChartData}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
            <XAxis dataKey="date" stroke="#9ca3af" fontSize={12} />
            ...
            <Tooltip
              ...
              formatter={(value: number) => [formatQuantity(value), "Usage"]}
              labelStyle={{ color: "#d1d5db" }}
            />
            {reposToShow.map((repo, index) => (
              <Bar
...
function SKUBasedChart({ data, title, serviceType }: ServiceChartProps) {
  ...
  const dailyData = data.reduce((acc, item) => {
    const date = item.date;
    if (!acc[date]) {
      acc[date] = { date, cost: 0, quantity: 0, skus: new Set(), organizations: new Set() };
    }
  ...
  const orgData = hasMultipleOrganizations ? data.reduce((acc, item) => {
  ...
  const orgPieData = hasMultipleOrganizations ? Object.values(orgData)
    .sort((a, b) => b.cost - a.cost) : [];
  ...
  return (
    <div className="space-y-6">
      {/* Summary Stats */}
      <div className={`grid grid-cols-1 ${hasMultipleOrganizations ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-4 mb-6`}>
        <div className="bg-gray-800/50 rounded-lg p-4">
          <h3 className="text-sm text-gray-400 mb-1">Total Cost</h3>
          <p className="text-2xl font-bold text-green-400">{formatCurrency(totalCost)}</p>
        </div>
        ...
        {hasMultipleOrganizations && (
          <div className="bg-gray-800/50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-orange-400">{uniqueOrganizations}</p>
          </div>
        )}
      </div>

      <div className={`grid grid-cols-1 ${hasMultipleOrganizations ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} gap-6`}>
        {/* Daily Cost Trend */}
            ...
            {skuPieData.map((entry, index) => (
              <div key={entry.sku} className="flex items-center text-sm">
                <div
                  ...
                  style={{ backgroundColor: COLORS[index % COLORS.length] }}
                />
                <span className="text-gray-300 flex-1 truncate" title={entry.sku}>{entry.sku}</span>
                <span className="text-white font-medium">{formatCurrency(entry.cost)}</span>
              </div>
            ))}
          </div>
        ...
        {hasMultipleOrganizations && (
          <div className="lg:col-span-3 bg-gray-800/30 rounded-lg p-6">
            <h3 className="text-lg font-semibold mb-4">Cost by Organization</h3>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                  ...
                  >
                    {orgPieData.map((entry, index) => (
                      <Cell key={`org-${index}`} fill={COLORS[(index + 6) % COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip
                    contentStyle={{
              ...
              <div className="space-y-3">
                <h4 className="text-md font-medium text-gray-300 mb-3">Organization Breakdown</h4>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {orgPieData.map((entry, index) => (
                    <div key={entry.organization} className="flex items-center justify-between text-sm bg-gray-700/30 rounded p-2">
                      <div className="flex items-center">
                        <div
                          className="w-3 h-3 rounded mr-3"
                          style={{ backgroundColor: COLORS[(index + 6) % COLORS.length] }}
                        />
                        <span className="text-white font-medium" title={entry.organization}>
                          {entry.organization}
                        </span>
                      </div>
                      <div className="text-right">
                        <div className="text-green-400 font-medium">{formatCurrency(entry.cost)}</div>
                        <div className="text-gray-400 text-xs">{formatQuantity(entry.quantity)}</div>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {/* Quantity Usage Chart */}
      <div className="bg-gray-800/30 rounded-lg p-6">
        <h3 className="text-lg font-semibold mb-4">Daily Usage Volume</h3>
        <ResponsiveContainer width="100%" height={300}>
